...
void fill_screen(byte r, byte g, byte b)
{
  tft.setColor(r, g, b);
  tft.fillRoundRect(0, 25, 320, 480);
}
...
void bmpDraw(char *filename, uint8_t x, uint16_t y, byte color_back) {
  ...
  if(read16(bmpFile) == 0x4D42) { // BMP signature
    ...
    if(read16(bmpFile) == 1) { // # planes -- must be '1'
      ...
      if((bmpDepth == 24) && (read32(bmpFile) == 0)) { 
        ...
        switch(color_back)
        {
          case 1: tft.setColor(255, 255, 255); break;
          case 2: tft.setColor(255,215,184);   break;
        }/*
        if(color_back != 3) tft.fillRoundRect(x,y,x+w-1,y+h);
        else tft.drawRoundRect(x,y,x+w-1,y+h);*/
        for (row=0; row<h; row++) {
          if(flip) 
